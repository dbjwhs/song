...
- **Multi-Client Support**: `run_tcp_multi()` accepts concurrent clients (thread-per-client), all sharing the same object registry and subscription fan-out. Concurrency is capped (`set_max_concurrent_clients()`, default 128) and finished workers self-reap, so an unauthenticated peer cannot force unbounded thread/fd growth. A TCP peer that starts a message then stalls is dropped after a per-message deadline (`TcpTransport::set_message_deadline_ms()`, default 30 s) so it cannot pin a worker indefinitely (slowloris).
